...
function validateCarForm(payload) {
  ...
  if (!payload || typeof payload.model !== 'string' || payload.model.length < 2) {
    isFormValid = false
    errors.model = 'Model must be more than 2 symbols.'
  }

  if (!payload || !payload.year || payload.year < 1900 || payload.year > 2020) {
    isFormValid = false
...
router.post('/buy/:id', authCheck, (req, res) => {
  ...
  carsData.buyCar(id, user)

  res.status(200).json({
    success: true,
    message: 'You bought the car successfully.'
  })
})
...
router.get('/userCars', authCheck, (req, res) => {
  const user = req.user.email
...
router.get('/getCar/:id', authCheck, (req, res) => {
  const id = req.params.id
